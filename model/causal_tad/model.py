...
class CausalTADModel(nn.Module):
    def __init__(self, input_size, hidden_size, device, layer_rnn, label_num) -> None:
        super().__init__()
        self.device = device
    ...
    def get_mask(self, edge_list, label, batch_size, seq_len):
        ...
        mask = torch.cat(
            (
                torch.ones(batch_size, 1, self.label_num).to(mask.device),
                mask[:, :-1, :],
            ),
            dim=1,
        )
    ...
    def forward(self, src, trg, edge_list, src_lengths, trg_lengths=None):
        """
        Input:
        ...
        src_lengths = torch.zeros([batch_size], device="cpu").long() + 2
