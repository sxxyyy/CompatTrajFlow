...
from utils.edge_remapper import build_grid_adj_dnorm, get_or_build_edge_mapping
...
class co_attention(nn.Module):
    def __init__(self, dim):
        ...
        self.FFN_s = nn.Sequential(
            nn.Linear(dim, dim), nn.ReLU(), nn.Linear(dim, dim), nn.Dropout(0.1)
        )
    ...
    def forward(self, seq_s, seq_t):
        ...
        att_s = self.layer_norm(
            self.FFN_s(torch.matmul(coatt_s, v_s)) + torch.matmul(coatt_s, v_s)
        )
        att_t = self.layer_norm(
            self.FFN_t(torch.matmul(coatt_t, v_t)) + torch.matmul(coatt_t, v_t)
        )

        return att_s.squeeze(2), att_t.squeeze(2)


class MSTOATD(nn.Module):
    def __init__(
        self, token_size: int, token_size_out, args: Namespace, generator: Generator
    ):
        super().__init__()

        self.emb_size = args.embedding_size
        self.mst_num_clusters = args.mst_num_clusters
        self.dataset = args.location
        self.s1_size = args.s1_size
        self.s2_size = args.s2_size

        self.pi_prior = nn.Parameter(
            torch.ones(args.mst_num_clusters) / args.mst_num_clusters
        )
        self.mu_prior = nn.Parameter(
            torch.randn(args.mst_num_clusters, args.hidden_size, generator=generator)
        )
        self.log_var_prior = nn.Parameter(
            torch.zeros(args.mst_num_clusters, args.hidden_size)
        )

        self.embedding = nn.Embedding(token_size, args.embedding_size)

        self.encoder_s1 = nn.GRU(
            args.embedding_size * 2, args.hidden_size, 1, batch_first=True
        )
        self.encoder_s2 = nn.GRU(
            args.embedding_size * 2, args.hidden_size, 1, batch_first=True
        )
        self.encoder_s3 = nn.GRU(
            args.embedding_size * 2, args.hidden_size, 1, batch_first=True
        )
        ...
        self.nodes = torch.arange(token_size, dtype=torch.long)

        use_grid = getattr(args, "use_grid_tokens", False)
        if use_grid:
            self.adj, self.d_norm = build_grid_adj_dnorm(
                args.grid_num_rows, args.grid_num_cols
            )
        else:
            raw_adj = sparse.load_npz("data/{}/raw/adj.npz".format(args.location))
            raw_d_norm = sparse.load_npz("data/{}/raw/d_norm.npz".format(args.location))

            edge_mapping = getattr(args, "edge_mapping", None)
            if edge_mapping is None:
                edge_mapping = get_or_build_edge_mapping(args.location)

            num_dense = max(token_size, max(edge_mapping.values()) + 1)
            mapped_indices = [0] * num_dense
            for raw_u, mapped_u in edge_mapping.items():
                mapped_indices[mapped_u] = raw_u

            self.adj = raw_adj.tocsr()[mapped_indices, :][:, mapped_indices].tocoo()
            self.d_norm = raw_d_norm.tocsr()[mapped_indices, :][
                :, mapped_indices
            ].tocoo()

        v_uniform_distribution = torch.distributions.uniform.Uniform(-0.2, 0.2)
        omega_uniform_distribution = torch.distributions.uniform.Uniform(-0.1, 0.1)

        if args.location == "porto":
            self.V = nn.Parameter(
                v_uniform_distribution.sample((token_size, token_size))
            )
        else:
            self.V = nn.Parameter(
                v_uniform_distribution.sample(
        ...
        self.co_attention = co_attention(args.embedding_size)
        self.d2v = TemporalVec(k=self.emb_size)

        self.w_omega = nn.Parameter(
            omega_uniform_distribution.sample(
                (args.embedding_size * 2, args.embedding_size * 2)
            )
        )
        self.u_omega = nn.Parameter(
            omega_uniform_distribution.sample((args.embedding_size * 2, 1))
        )
    ...
    def scale_process(self, e_inputs, scale_size, lengths):
        ...
        e_inputs_split = pack_padded_sequence(
            e_inputs_split, lengths, batch_first=True, enforce_sorted=False
        )
    ...
    def getMask(self, seq_lengths):
        ...
        mask = torch.ones((len(seq_lengths), max_len))
    ...
    def forward(
        self, trajs: Tensor, times: Tensor, lengths: Tensor, batch_size, mode, c
    ):
        float_times = times.to(torch.float)
        # spatial embedding
        ...
        s_inputs = torch.index_select(nodes, 0, trajs.flatten()).reshape(
            batch_size, -1, self.emb_size
        )
        # temporal embedding
        t_inputs = self.d2v.encode(float_times)

        att_s, att_t = self.co_attention(s_inputs, t_inputs)
        st_inputs = torch.concat((att_s, att_t), dim=2)
        d_inputs = torch.cat(
            (
                torch.zeros(batch_size, 1, self.emb_size * 2, dtype=torch.long),
                st_inputs[:, :-1, :],
            ),
            dim=1,
        )  # [bs, seq_len, emb_size * 2]

        decoder_inputs = pack_padded_sequence(
            d_inputs, lengths, batch_first=True, enforce_sorted=False
        )

        if mode == "train":
            encoder_inputs_s1 = pack_padded_sequence(
                self.attention_layer(st_inputs, lengths),
                lengths,
                batch_first=True,
                enforce_sorted=False,
            )
            encoder_inputs_s2 = self.scale_process(
                st_inputs, self.s1_size, [int(i // self.s1_size) for i in lengths]
            )
            encoder_inputs_s3 = self.scale_process(
                st_inputs, self.s2_size, [int(i // self.s2_size) for i in lengths]
            )
            ...
            encoder_final_state = (
                self.W1 * encoder_final_state_s1
