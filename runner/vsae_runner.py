...
from model.vsae.model import VSAE
from runner.abstract_runner import AbstractRunner

logger = logging.getLogger(__name__)


class VSAERunner(AbstractRunner):
    def __init__(self, args: Namespace, generator: Generator) -> None:
        super().__init__(args, generator)
        self.name = "VSAE"
        self.model = VSAE(self.num_edges, args)
        self.optimizer = Adam(self.model.parameters(), lr=args.vsae_learning_rate)
    ...
    def create_checkpoint_dir(self):
        checkpoint_dir = f"{self.checkpoint_path}/{self.location}/vsae"
        os.makedirs(checkpoint_dir, exist_ok=True)
        logger.info("Checkpoint directory created at: %s", checkpoint_dir)

    def save_checkpoint(self):
        checkpoint_path = f"{self.checkpoint_path}/{self.location}/vsae/model.pt"
